...
#include <assert.h>
#include <errno.h>
#include "detect_saccade_for_R.h"
...
/* VECTOR TO DEGREE FUNCTION AS AN R INTERFACE */
void vec_to_degree_for_R(double *x_here, double *y_here, double *deg_here) {
    deg_here[0] = atan2(y_here[0], x_here[0])*180*1.0/M_PI;
    if (deg_here[0] < 0) {
        deg_here[0] = deg_here[0] + 360;
    }
}
...
void run_detection(
                   ...
                   double *detection_results_array
                  ) {
    // output
    ...
    ncols_x = ncols_y = ncols_t = n_elements[0];
    ...
    if (print_results[0] != 0) {
        ...
        fprintf(stdout, "thres_fac=%i above_thres_needed=%i restrict_dir_min=%i restrict_dir_max=%i sampling_freq=%i anchor_vel_thres=%i print_results=%i n_samples=%i\n",
                thres_fac[0], above_thres_needed[0], restrict_dir_min[0], restrict_dir_max[0], sampling_freq[0], anchor_vel_thres[0], print_results[0], ncols_x);
    }
    ...
    if (sampling_freq[0] > 0) { // we interpolate based on the given sampling rate
        fd_uniform = 1000 / (double) sampling_freq[0]; // frame duration in milliseconds
...
//        }
    ...
    threshold_vx = sd_vx * (double) thres_fac[0];
    threshold_vy = sd_vy * (double) thres_fac[0];
    if (print_results[0] != 0) {
...
//        }
        ...
        for (it=ncols_v-above_thres_needed[0]; it<ncols_v; it++) { 
            ...
            if (test_criterion == 1) {
                ...
                // do we check the direction criterion?
                if (restrict_dir_min[0]==0 || restrict_dir_max[0]==0) {
                    ...
                    if (restrict_dir_max[0] > restrict_dir_min[0]) { // any saccade
                        if ( deg < (double) restrict_dir_max[0] && 
                                deg > (double) restrict_dir_min[0] ) {
                            direction_criterion = 1;
                        } else {
                            direction_criterion = 0;
                        }
                    } else { // rightward saccade
                        if ( (deg >= 0.0 && deg < (double) restrict_dir_max[0]) ||
                                (deg <= 360.0 && deg > (double) restrict_dir_min[0]) ) {
                            direction_criterion = 1;
                        } else {
                            direction_criterion = 0;
                        }
        ...
        double anchor_thres_vx = sd_vx * (double) anchor_vel_thres[0];
        double anchor_thres_vy = sd_vy * (double) anchor_vel_thres[0];
        sac_t_onset = NAN;       
        if (sac_detected == 1) {
            if (print_results[0] != 0) {
    ...
    detection_results_array[0] = sac_detected;
    detection_results_array[1] = sac_t;
    detection_results_array[2] = sac_vx;
    detection_results_array[3] = sac_vy;
    detection_results_array[4] = threshold_vx;
    detection_results_array[5] = threshold_vy;
    detection_results_array[6] = sac_t_onset;
    detection_results_array[7] = deg;
};
